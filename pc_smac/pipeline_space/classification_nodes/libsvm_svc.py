...
class LibSVM_SVC_Node(Node):

    def __init__(self):
        self.name = "libsvm_svc"
        self.type = "classifier"
        self.hyperparameters = {"C": 1.0, "kernel": "rbf", "degree": 3, "gamma": 0.1, "coef0": 0, "shrinking": "True",
                                "tol": 1e-4, "max_iter": -1}
        self.algorithm = LibSVM_SVC

    def initialize_algorithm(self, hyperparameters):
        hyperparameters = self.initialize_hyperparameters(hyperparameters)
        libsvm_svc = self.algorithm(C=hyperparameters["C"], # actual hp
                                    kernel=hyperparameters["kernel"], # actual hp
                                    degree=hyperparameters["degree"], # actual hp
                                    gamma=hyperparameters["gamma"], # actual hp
                                    coef0=hyperparameters["coef0"], # actual hp
                                    shrinking=hyperparameters["shrinking"], # actual hp
                                    tol=hyperparameters["tol"], # actual hp
                                    max_iter=hyperparameters["max_iter"]
                                    )

        return (self.get_full_name(), libsvm_svc)
...
class LibSVM_SVC(ClassificationAlgorithm):

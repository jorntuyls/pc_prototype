...
class KNearestNeighborsNode(Node):

    def __init__(self):
        self.name = "k_nearest_neighbors"
        self.type = "classifier"
        self.hyperparameters = {"n_neighbors": 1, "weights": "uniform", "p": 2}
        self.algorithm = KNearestNeighbors

    def initialize_algorithm(self, hyperparameters):
        hyperparameters = self.initialize_hyperparameters(hyperparameters)
        k_nearest_neighbors = self.algorithm(n_neighbors=hyperparameters["n_neighbors"],
                                             weights=hyperparameters["weights"],
                                             p=hyperparameters["p"])

        return (self.get_full_name(), k_nearest_neighbors)
...
class KNearestNeighbors(ClassificationAlgorithm):

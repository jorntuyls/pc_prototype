...
class MultinomialNBNode(Node):

    def __init__(self):
        self.name = "multinomial_nb"
        self.type = "classifier"
        self.hyperparameters = {"alpha": 1, "fit_prior": "True"}
    ...
    def initialize_algorithm(self, hyperparameters):
        ...
        multinomial_nb = self.algorithm(alpha=hyperparameters["alpha"],
                                        fit_prior=hyperparameters["fit_prior"])

        return (self.get_full_name(), multinomial_nb)
...
class MultinomialNB(ClassificationAlgorithm):
